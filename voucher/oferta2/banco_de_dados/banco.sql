...
select count(CODCID) from cidade; -- 1
select count(CODCLI) from cliente; -- 2
select count(CODPROD) from produto; -- 3
select count(CODSETOR) from setor; -- 4
select count(NUMPED) from pedido; -- 5
select max(SALARIO) from vendedor; -- 6
select * from vendedor where SALARIO = (select max(SALARIO) from vendedor); -- 7
select min(SALARIO) from vendedor; -- 8
select * from vendedor where SALARIO = (select min(SALARIO) from vendedor); -- 9
select * from pedido where NUMPED = (select NUMPED from itempedido where QTDADE = (select max(QTDADE) from itempedido)); -- 10
select * from pedido where NUMPED = (select NUMPED from itempedido where QTDADE = (select min(QTDADE) from itempedido)); -- 11
select * from produto where VALOR_UN = (select min(VALOR_UN) from produto); -- 12
select * from produto where VALOR_UN = (select max(VALOR_UN) from produto); -- 13
select vendedor.NOMEVEND,vendedor.SALARIO,setor.NOMESETOR from vendedor join setor on vendedor.CODSETOR = setor.CODSETOR where setor.NOMESETOR = "FERRAMENTAS"; -- 14

select pedido.ENTREGA,pedido.CODVEND,cliente.NOME from pedido join cliente on pedido.CODCLI = cliente.CODCLI where cliente.NOME = "Estevan Pereira Cardoso"; -- 15
