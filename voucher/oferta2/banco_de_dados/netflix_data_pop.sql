...
insert into tipo_video (tipo_v_valor) values 
 ...
 create table assistido (assist_id int primary key auto_increment, assist_cliente_id int, assist_avaliacao tinyint not null, assist_tipo_v tinyint not null, assist_conteudo_id int,
 ...
 create table video (v_id int primary key auto_increment, v_tipo_v tinyint, foreign key (v_tipo_v) references tipo_video(tipo_v_id),
 v_film_id int, foreign key (v_film_id) references filme(film_id), v_serie_id int, foreign key (v_film_id) references filme(film_id),
 v_doc_id int, foreign key (v_doc_id) references documentario(doc_id));
 
 create table temporada(temp_id int primary key auto_increment, temp_numero smallint not null, temp_serie_id int,
 foreign key (temp_serie_id) references serie(serie_id));
 
 alter table serie_ep drop column ep_serie_id;
